...
fn get_outfile() -> std::fs::File {
    ...
    if path::Path::new("./results/free_users.txt").exists() {
        fs::rename("./results/free_users.txt", format!("./results/free_users_{:?}.txt", rand::thread_rng().gen_range(0..100))).expect("Couldnt rename existing `free_users.txt` file");
        return File::create("./results/free_users.txt").unwrap();
    } else {
        return File::create("./results/free_users.txt").unwrap();
    }
}
